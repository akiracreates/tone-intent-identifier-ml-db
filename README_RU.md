...
# Система идентификации тона и намерения сообщения (ML + БД)
...
-   **машинное обучение** для классификации **тона** и
    **коммуникативного намерения** коротких текстовых сообщений;\
-   **реляционную базу данных PostgreSQL** для хранения сообщений,
    разметки, предсказаний моделей и аналитических SQL-запросов.
...
-   принимать текстовые сообщения (например, небольшие сообщения,
    комментарии, письма);\
-   определять два параметра:
    -   **Tone (тон сообщения)**\
    -   **Intent (коммуникативное намерение)**\
-   сохранять сообщения, истинные метки и предсказания в
    **PostgreSQL**;\
...
-   Новички, которые хотят увидеть:
    -   понятный ML-пайплайн на классических методах (TF-IDF, Logistic
        Regression и т.д.);
...
Проект использует **два отдельных классификатора**, формируя **два
независимых предсказания**: тон и намерение.

### 2.1. Метки тона

1.  `neutral` -- нейтральный\
2.  `positive_friendly` -- дружелюбный, позитивный\
3.  `negative_rude` -- грубый, негативный\
4.  `sarcastic_ironic` -- саркастичный, ироничный\
5.  `formal` -- формальный\
...
-   **ML-библиотеки:** pandas, numpy, scikit-learn, matplotlib/seaborn
    (опционально)\
...
-   Сервер знакомого\
-   Недорогой VPS (Hetzner, Contabo, DigitalOcean)\
